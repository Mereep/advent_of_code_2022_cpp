 ...
 * a field stores if it was already visited and the distance travelled to it (lowest distance)
 * **/
struct Field {
...
struct HeightMap {
    ...
    static HeightMap fromInput(const std::vector<std::string>& lines) {
        HeightMap map = {
            .fields = {},
            .width = lines[0].size(),
            .height = lines.size()
        ...
        for(const auto& line: lines) {
            for (const auto& h: line) {
                if (h == 'S') {
                    map.fields.push_back(std::make_shared<Field>(Field {
    ...
    [[nodiscard]] std::shared_ptr<Field> get_field(size_t x, size_t y) const {
        return fields[y * width + x];
    }

    [[nodiscard]] std::shared_ptr<Field> get_field_at_index(size_t index) const {
        return fields[index];
    }
    ...
    [[nodiscard]] size_t get_start_field_pos() const {
        for (size_t i = 0; i < fields.size(); i++) {
            if (fields[i]->is_start) {
                return i;
            }
        }
        throw std::runtime_error("no start field found");
    }

    // transforms a flat index to coordinates
    [[nodiscard]] std::tuple<size_t, size_t> index_to_coordinates(size_t index) const {
...
std::string Day12::part1() {
    ...
    map.run();
    return std::to_string(map.global_shortest_path);
}
...
std::string Day12::part2() {
    ...
    std::vector<size_t> all_a_positions = {};
    ...
    unsigned long min_path = -1; // we could use the result we know from part 1 to maybe save a little bit of time
